...
class SubCategorys extends MX_Controller {
    ...
    public function index(){
        ...
        $this->load->model('subcategorys/subcategorys_model');
        $this->template->write('website_name', 'Amazing Chaiyaphum : จัดการหมวดหมู่ย่อย');
        $this->data->Header_title = 'จัดการหมวดหมู่ย่อย';
        $this->data->title = 'รายการหมวดหมู่ย่อยทั้งหมด';
        $this->data->mainCategorys = $this->subcategorys_model->getMainCategory();
...
//        $this->template->add_js(URLPATH_JS.'plugins/dataTables/dataTables.bootstrap.js');
        ...
        // $total_rows = $this->subcategorys_model->getAllCategorys
        ...
        $base_url = base_url().'subcategorys';
        $uri_segment =3;
        $per_page = 20;
        $start = (($per_page*$page)-$per_page);
        $total_rows = $this->subcategorys_model->getAllCategorys(1);
        ...
        $this->data->allCategorys = $this->subcategorys_model->getAllCategorys(0,$start,$per_page);
        $this->data->pagination = $this->pagination_page($base_url,$uri_segment,$per_page,$total_rows);

        
//        
        // $this->data->allCategorys = $this->subcategorys_model->getAllCategorys();
        # set active menu
       $this->data->main_active = 'subcategorys';
//        $this->data->menu_active = 'system_management';
        # start write view to template
        $this->template->write_view('menu_top','menu',$this->data); 
        $this->template->write_view('fullpage','subcategorys/page_main',$this->data);
        $this->template->render(); 
    }
    public function add(){
        $this->template->write('website_name', 'Amazing Chaiyaphum : จัดการหมวดหมู่ย่อย');
        $this->data->Header_title = 'เพิ่มหมวดหมู่ย่อย';
       ...
       $this->template->add_css(URLPATH_ADMIN_CSS.'cropper.css');*/
        if ($this->input->post()) {
            ...
            $error = '';$error_val = '';
            if ($ddmaincategory == '0' || $ddmaincategory ==null){
                $error['ddmaincategory'] = 'กรุณาเลือกหมวดหมู้หลัก'; 
            }else{
                  $error_val['val_ddmaincategory'] = $ddmaincategory;
            }
            ...
            if ($error == '') {
                ...
                if ($id = $this->subcategorys_model->addCateData($data_set)) {
                    if ($id != 0) {
                        if ($thumb_name = $this->upload_image($thumb,'cate',$id)) {
                            $thumb_data = array('image_thumb_vertical'=>$thumb_name);
                            $this->subcategorys_model->updateImageThumbName($thumb_data,$id);
                        }
        ...
        $this->template->write_view('menu_top','menu',$this->data); 
        $this->template->write_view('fullpage','subcategorys/add_form',$this->data);
        $this->template->render(); 
    }
    public function edit($cate_id=0){
        ...
        $this->data->main_active = 'subcategorys';
        $this->data->mainCategorys = $this->subcategorys_model->getMainCategory();
        $this->data->catedata = $this->subcategorys_model->getCategoryByID($cate_id);
        if ($this->input->post()) {
            $ddmaincategory = $this->input->post('ddmaincategory');
            $txtname = $this->input->post('txtname');
            $txtname_en = $this->input->post('txtname_en');
            ...
            if ($error == '') {
                ...
                if ($this->subcategorys_model->updateCateData($data_set,$cate_id)) {
                    if ($cate_id != 0) {
                        if ($thumb['name'] !='' && $thumb['name']!=null) {
                            if ($thumb_name = $this->upload_image($thumb,'cate',$cate_id)) {
                                $thumb_data = array('image_thumb_vertical'=>$thumb_name);
                                $this->subcategorys_model->updateImageThumbName($thumb_data,$cate_id);
                            }    
                        }
                        ...
                        # parse value msg to show error or succsess data for a while. 
                        $this->session->set_flashdata('msg',$data);
                        redirect('subcategorys/edit/'.$cate_id,'refresh');
                        exit;
                    }
        ...
        $this->template->write_view('menu_top','menu',$this->data); 
        $this->template->write_view('fullpage','subcategorys/edit_form',$this->data);
        $this->template->render(); 
    }
    private function upload_image($file,$type,$id){
            ...
            $file_path = URLPATH_UPLOAD_SUBCATEIMAGES.$id.'/';
            if (!is_dir($file_path)) {
            ...
            if (in_array($ext, $arr_typefiles)) {
               ...
               if (move_uploaded_file($file['tmp_name'], $file_path . $filename)) {
                    if ($type=='pincate') {
                        ...
                        if($this->resizeImage($file_path . $filename,400,300)){
                            return $filename;
                        }else{
                            return false;
                        }
    ...
    public function delete(){
        ...
        if ($id!='') {
            $this->load->model('subcategorys/subcategorys_model');
            if ($this->subcategorys_model->delete($id)) {
                echo 1;
            }else{
                echo 0;
            }
    ...
    public function get_AutocompleteCate(){
        ...
        $this->load->model('subcategorys/subcategorys_model');
        $rawdata = $this->subcategorys_model->getautocompleteCate($string);
        $source = array();
